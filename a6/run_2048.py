...
class Tile(Canvas):
    ...
    def __init__(self, master, width=0, height=0, **kwargs):
        ...
        self.textObj = self.create_text(
            width / 2, height / 2, font=kwargs["font"], text=" "
        )
...
class Game2048:
    def __init__(self, master, num_rows, num_cols):
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                self.tiles[(i, j)] = Tile(
                    master, width=w, height=h, padx=40, pady=40, font=("Helvetica", 32)
                )
    ...
    def keypress(self, event):
        k = event.char.lower()
        if k in ["w", "a", "s", "d", "h", "j", "k", "l"]:  # add vim keybinding
